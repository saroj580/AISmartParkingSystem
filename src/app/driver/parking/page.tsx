...
export default function NearbyParkingPage() {
  ...
  const results = useMemo(() => {
    let lots = PARKING_LOTS.filter((lot) =>
      `${lot.name} ${lot.city} ${lot.addressLine}`
        .toLowerCase()
        .includes(query.toLowerCase()),
    );
    if (vehicleType !== "ALL") {
      lots = lots.filter((lot) => lot.capacity[vehicleType].total > 0);
    }
  ...
  }, [query, vehicleType, sort]);

  return (
    <div className="mx-auto flex max-w-6xl flex-col gap-6">
      <PageHeader
        title="Nearby parking"
        description={`${results.length} lots found near San Francisco, CA`}
      />
        ...
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
          {results.map((lot) => (
            <ParkingLotCard
              key={lot.id}
              lot={lot}
              saved={CURRENT_DRIVER.savedLots.includes(lot.id)}
            />
          ))}
        </div>
      )}
